...
import { ArrowLeft, Settings, Ticket, RefreshCw } from "lucide-react";

const ManageQueue = () => {
  const { queueId } = useParams<{ queueId: string }>();
  const { user, loading: authLoading } = useAuth();
  const { queue, loading: queueLoading, updateQueue, incrementServing, toggleSystemStatus } = useQueue(queueId);
